...
from mood_tagger.data.sets import SnippetDataset, FramesToSamples
...
class Entry:
    ...
    def get_gems_9(self):
        return self.data_row[GEMS_9].to_numpy()
...
class RandomPartitioner:
    def __init__(self, feats, annots, targs, names, seed, test_split=0.15, valid_split=0.15):
        self.feats = feats
        self.annots = annots
        self.targs = targs
        self.names = names
        ...
        assert num_samples == len(annots) == len(targs) == len(names)
...
def load_data(
    ...
    partitioner = RandomPartitioner(feats, annots, targs, names, 42)

    valid_subset = None
    if val_size is not None and 0.0 < val_size < 1.0:
        valid_subset = val_size
    pin_memory = gpu_num > -1
    train_feats, train_annots, train_targs, train_names = partitioner.get_split('train')
    ...
    if valid_fm_thresh > 0:  # we need this so we know which annotation to use, also for RNNs is more true
        valid_seq_len = -1
        valid_batch_size = 1
    else:
        valid_seq_len = seq_len
        valid_batch_size = batch_size
    valid_feats, valid_annots, valid_targs, valid_names = partitioner.get_split('valid')
    ...
    test_feats, test_annots, test_targs, test_names = partitioner.get_split('test')
