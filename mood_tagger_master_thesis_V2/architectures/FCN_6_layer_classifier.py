    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cnn = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=1, out_channels=channel_base, kernel_size=3 , stride=stride, padding= 1 ),# (3, 3)
            torch.nn.ELU(),
            torch.nn.BatchNorm2d(channel_base),
            torch.nn.MaxPool2d((2, 1)), #200 #150

            torch.nn.Conv2d(in_channels=channel_base, out_channels=channel_base*2, kernel_size=3, stride=stride, padding=1),
            torch.nn.ELU(),
            torch.nn.BatchNorm2d(channel_base*2),
            torch.nn.MaxPool2d((2, 3)),
            torch.nn.Dropout2d(self.dropout_p),

            torch.nn.Conv2d(in_channels=channel_base*2, out_channels=2*channel_base, kernel_size=3, stride=stride, padding=1),
            torch.nn.ELU(),
            ...
            torch.nn.Conv2d(in_channels=channel_base*2, out_channels=1*channel_base, kernel_size=3, stride=stride, padding=1),
            torch.nn.ELU(),
        ...
        self.dense = torch.nn.Linear(channel_base, num_classes)
