    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cnn = torch.nn.Sequential(
            ...
            ######################################
            torch.nn.Conv2d(
                in_channels=8 * channel_base, out_channels=16 * channel_base, kernel_size=(3, 3), padding="same"
            ),
            torch.nn.ELU(),
            ...
            torch.nn.Conv2d(
                in_channels=16 * channel_base, out_channels=16 * channel_base, kernel_size=(3, 3), padding="same"
            ),
            torch.nn.ELU(),
            torch.nn.BatchNorm2d(16 * channel_base),
            torch.nn.Dropout2d(self.dropout_p),
            ...
            ######################################
            torch.nn.Conv2d(
                in_channels=16 * channel_base, out_channels=32 * channel_base, kernel_size=(3, 3), padding="same"
            ),
            torch.nn.ELU(),
            torch.nn.BatchNorm2d(32 * channel_base),
            torch.nn.Dropout2d(self.dropout_p),

            torch.nn.Conv2d(
                in_channels=32 * channel_base, out_channels=32 * channel_base, kernel_size=(3, 3), padding="same"
            ),
            torch.nn.ELU(),
            torch.nn.BatchNorm2d(32 * channel_base),
            torch.nn.Dropout2d(self.dropout_p),
...
##################################
            ...
            torch.nn.Conv2d(in_channels=32 * channel_base, out_channels=self.num_classes, kernel_size=(1, 1), padding="same"),
            torch.nn.ELU(),
